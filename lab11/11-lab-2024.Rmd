...
sb_locs_state <- full_join(usa_pop_abbr, sb_locs_state, by = "Abbreviation")
sb_locs_state
```
```{r}
...
```{r message=FALSE}
p1 <- ggplot(sb_locs_state, aes(x = population, y = Count)) +
  geom_point() +
  theme_minimal() +
...
p2 <- ggplot(sb_nutr, aes(x = Calories, fill = Category)) +
  ...
  theme_minimal() +
  labs(x = "Calories", y = "Count", title = "Caloric Distribution of Starbucks Menu Items")
